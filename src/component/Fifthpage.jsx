...
function Fifthpage({ onNext ,onHome}) {
  const options = [
  ...
  return (
    <div className='flex flex-col min-h-full bg-[url("/background.svg")] bg-no-repeat bg-cover items-center relative overflow-hidden text-black'>
      ...
      <div className="flex flex-col font-inter-custom items-start w-full gap-14 px-28 h-[40%]">
        ...
        {hasSelection && (
          <button
            onClick={handleNext}
            type="button"
            className="py-8 px-10 w-[20rem] font-inter-custom text-4xl font-medium text-gray-900 bg-transparent rounded-full border border-black/20 hover:bg-gray-100 hover:text-black focus:outline-none focus:ring-4 focus:ring-gray-100 transition"
          >
            Next
          </button>
        )}
